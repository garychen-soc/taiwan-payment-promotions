...
全支付目前會從官方「百大品牌」與「夏日乾杯飲料季」主題總覽 JSON 發現活動，再沿官方活動詳情的 EventId 連結圖補齊相關活動；每筆活動同時查詢官方詳情 API 與頁面使用的公開額滿狀態 API，並讀取官網最新消息 JSON。這些主題總覽不是全站完整清單，所以報表會誠實標為 `partial_public_activity_discovery`，並由每日 AI 官方網域搜尋補查。全支付 App 小鈴鐺的「活動通知」也沒有公開列表，因此只有 App 能查到的額滿狀態會繼續標為 `unknown_app_only`，不會推測仍有名額。報表中的來源成功率代表已登錄來源可讀取，不代表 App-only 活動已達絕對完整涵蓋。
